...
- 卡片复用：后续问答若在答案中引用 `[卡片id]`，该卡片 `use_count` 自动 +1，成长页和卡片页可见。
- 每日审核：每天 23:05 自动生成本日已沉淀内容、错题本和调整方案。

问答检索使用“成长感知混合排序”：

1. 语义相似度占主权重；
2. 标题、摘要、正文和结构化经验字段做关键词兜底；
3. `practice`、被正确复验卡片引用的笔记和高频笔记获得小幅加权；
4. 已沉淀知识卡片可回流到 Agent 上下文；
5. 只有最终答案中真实出现 `[note_id]` 引用时才累计调用频次。
